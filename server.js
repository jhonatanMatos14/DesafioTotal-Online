...
io.on('connection', socket => {
  ...
  // Guests send actions to the host. The host is the authoritative game engine.
  socket.on('playerAction', action => {
    ...
    // The first state is published by the host immediately after starting.
    // Until then, do not accept guest actions.
    if (!Number.isInteger(Number(currentPlayer))) {
      return socket.emit('errorMessage', 'Aguarde a sincronização da partida.');
    }

    if (Number(currentPlayer) !== playerIndex) {
  ...
  // Only the host may publish the authoritative game state.
  ...
  socket.on('disconnect', () => {
    ...
    // If a player leaves, keep the current turn valid for the remaining players.
