...
io.on('connection', socket => {
  ...
  socket.on('startGame',()=>{const room=getRoom(socket);if(!room||room.hostId!==socket.id||room.started)return;if(room.players.length<2)return socket.emit('errorMessage','Entre com pelo menos 2 jogadores.');room.started=true;room.state={jogadorAtual:0,partidaTerminou:false};room.lastPublishedState=null;io.to(room.code).emit('gameStarted',{players:roomInfo(room).players});});
